{{-- <div class="flex flex-col p-2 mx-auto max-w-lg text-center text-white bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 md:p-6 lg:p-4 xl:p-6 dark:bg-gray-800 dark:text-white">
    <h3 class="mb-4 text-3xl text-orange-600 font-bold">{{ $title }}</h3>
    <p class="font-light text-gray-600 sm:text-lg dark:text-gray-400 flex-1">{{ $description }}</p>
    <div class="flex justify-center items-baseline my-8">
        <span class="mr-2 text-4xl text-orange-600 font-extrabold">Rp.{{ $price }}</span>
...
<div class="flex flex-col p-2 mx-auto max-w-lg text-center text-white bg-white rounded-lg border border-gray-100 shadow dark:border-gray-600 md:p-6 lg:p-4 xl:p-6 dark:bg-gray-800 dark:text-white transform hover:scale-105  hover:bg-orange-600 group hover:shadow-xl hover:text-white hover:border-net-blue-50 hover:z-10 transition-all duration-300 ease-in-out">
    <h3 class="mb-4 text-3xl text-orange-600 font-bold group-hover:text-white">{{ $title }}</h3>
    <p class="font-light text-gray-600 sm:text-lg dark:text-gray-400 flex-1 group-hover:text-white">{{ $description }}</p>
    <div class="flex justify-center items-baseline my-8">
        <span class="mr-2 text-4xl text-orange-600 font-extrabold group-hover:text-white">Rp.{{ $price }}</span>
        <span class="text-orange-500 dark:text-gray-400 group-hover:text-white">/bulan</span>
    </div>
    <!-- List -->
    <ul role="list" class="mb-8 space-y-4 text-left flex-1">
        @foreach ($features as $feature)
    ...
    class="text-white bg-gradient-to-br from-net-blue-50 to-net-blue-50 hover:bg-primary-700 focus:ring-4 focus:ring-primary-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:text-white dark:focus:ring-primary-900"
    >
     Pesan Sekarang
 </a>
